                  ...
                  <div className="md:col-span-8">
                    ...
                    <ul className="space-y-2">
                      {exp.details.map((detail, idx) => (
                        <li key={idx} className="text-neutral-800 dark:text-neutral-300 leading-relaxed pl-4 relative before:content-[''] before:absolute before:left-0 before:top-2.5 before:w-1.5 before:h-1.5 before:bg-neutral-300 dark:before:bg-neutral-600 group-hover:before:bg-jap-red dark:group-hover:before:bg-jap-red before:transition-colors before:duration-300">
                          {detail}
                        </li>
                      ))}
                    </ul>
               ...
               <div className="space-y-8">
                 {EDUCATION.map((edu, idx) => (
                   <div key={idx} className="group hover:bg-neutral-50 dark:hover:bg-neutral-900/50 p-4 -mx-4 rounded transition-colors">
                     ...
                     <p className="text-neutral-700 dark:text-neutral-300">{edu.degree}</p>
                     {edu.details && <p className="text-sm text-neutral-500 dark:text-neutral-500 mt-1">{edu.details}</p>}
                   </div>
                 ))}
               </div>
               ...
               <ul className="space-y-4">
                 {CERTIFICATIONS.map((cert, idx) => (
                   <li key={idx} className="flex flex-col group hover:bg-neutral-50 dark:hover:bg-neutral-900/50 p-3 -mx-3 rounded transition-colors">
                     <span className="font-bold text-jap-black dark:text-jap-white group-hover:text-jap-red transition-colors">{cert.name}</span>
              ...
              <ul className="space-y-6">
                {AWARDS.map((award, idx) => (
                  <li key={idx} className="relative pl-6 before:content-['✦'] before:absolute before:left-0 before:text-jap-red before:text-xs before:top-1 group">
                    <p className="leading-snug text-neutral-800 dark:text-neutral-300 group-hover:text-jap-black dark:group-hover:text-white transition-colors">{award}</p>
                  </li>
                ))}
              </ul>
